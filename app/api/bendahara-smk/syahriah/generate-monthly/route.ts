...
// POST auto-generate Syahriah transactions for all active SMK santri
export async function POST(request: NextRequest) {
  try {
    ...
    // Build where clause for SMK santri
    ...
    // Get all active SMK santri matching criteria
    ...
    for (const santri of santriList) {
      ...
      try {
        ...
        const transaction = await prisma.transaksi.create({
          data: {
            kode: `SYH-SMK-${tahunKode}-${bulan}-${santri.nis}-${Date.now().toString().slice(-4)}`,
            santriId: santri.id,
            jenis: "SYAHRIAH",
            bulan: bulan,
    ...
    console.error("Error in generate monthly Syahriah:", error)
    return NextResponse.json(
      { error: "Failed to generate monthly Syahriah transactions" },
      { status: 500 }
    )
  }
}
